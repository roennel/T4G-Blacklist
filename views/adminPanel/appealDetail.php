<?php if($data->appeal->done == '1'): ?>
  
  Currently Broken, check back later, kthxbye
...
<?php if($data->appeal->done == '0' && $data->source[0] == 'G-Ban Import'): ?>
...
<div class="sub extended">
  ...
  <table class="data submissionTable" style="width: 100%">
    <thead>
      <tr>
        <th colspan="99">Appeal Detail <a style="float:right; color:inherit" href="#" onclick="t4g.url.action='adminLog'; t4g.url.query.set('value',<?php echo $data->appeal->appealId ?>); t4g.url.redirect();">View Log</a></th>
      </tr>
    </thead>
    <tbody>
      <tr>
      ...
      <tr>
        ...
        <td>
          ...
          <table class="data" style="width: 100%; margin-top: 10px">
            ...
            <tbody>
              <?php while($item = $data->appealAdminVoteReasons->fetch()): if($data->appeal->done != '1' and $data->valid) { $msg = 'Hidden'; } else { $msg = $item->message; } ?>
              <tr>
                <td style="width: 12.5%; vertical-align: top" title="<?php echo $item->date ? date('d.m.Y H:i:s', $item->date) : '' ?>"><?php echo $item->date ? date('d.m.Y', $item->date) : '' ?></td>
                <td style="width: 15%; vertical-align: top"><?php echo getUser($item->userId)->username ?></td>
      ...
      <tr>
        <td>Result</td>
        <td><?php echo $data->banned ? 'Banned' : 'Invalid' ?></td>
      </tr>
      <?php endif ?>
      
      <?php if($data->appeal->done != '1' && isAdmin()): ?>
      <tr>
        <td colspan="99">
          <div class="buttons-wrap">
            ...
            <div class="buttons buttons-misc" style="float:right">
              <input type="button" id="close" value="Close Appeal" />
              <select id="changeType" <?php echo $data->appealAdminVoteCount > 0 && !isTech() ? 'disabled="disabled"' : '' ?>>
...
window.addEvent('domready', function()
{
  ...
  <?php if(!$data->valid or (isMod() && !isAdmin()) or $data->appeal->done == '1'): ?>
  ...
  votes.each(function(data, tag)
  {
    if($(tag))
    {
      $(tag).addEvent('click', function()
      {
        ...
        }).get('appealId=<?php echo $data->appeal->appealId ?>&state=' + data[0] + '&msg=' + encodeURIComponent($('voteMessage').get('value')));
      }); 
    }
  });
  ...
  $('changeType').addEvent('change', function()
  {
    ...
    }).get('appealId=<?php echo $data->appeal->appealId ?>&type=' + selType.get('value'));
  });
  
  $('close').addEvent('click', function()
  {
    var conf = confirm("Are you sure you want to mark this appeal as invalid?");
    if(!conf) return;
    ...
    }).get('appealId=<?php echo $data->appeal->appealId ?>&msg=' + encodeURIComponent($('voteMessage').get('value')));
  });
